...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const { data, error } = await supabaseAdmin
		...
		.update({ status: 'active', disconnected_at: null, connected_at: new Date().toISOString() })
		.eq('id', connectionId)
	...
	if (error || !data) return json({ error: 'Not found or not reconnectable' }, { status: 404 });
	locals.audit.record('connection.reconnected', {
		subjectId: data.id
	});
