...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	let query = locals.supabase
		.from('products')
		.select(
			'id, brand_id, season_id, product_year, style_number, name, wholesale_price, category, ats, archived_at, product_variants(id, color, size, price_override), product_images(id, is_primary, sort_order)',
			{ count: 'exact' }
		)
