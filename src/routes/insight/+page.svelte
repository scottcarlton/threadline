<script lang="ts">
	...
	const showSummary = $derived(data.showSummary ?? []) as {
		showDateId: string;
		appointments: number;
		orders: number;
		revenue: number;
	...
	const columnGroups = $derived.by(() => {
		...
		for (const d of deliveries) {
			const label =
				d.delivery_month >= 1 && d.delivery_month <= 6 ? 'Spring / Summer' : 'Fall / Holiday';
	...
	const totalCommissions = $derived(
		commissionRows.reduce(
			(sum: number, r: { commissionAmount: number }) => sum + r.commissionAmount,
			0
		)
	);
...
{:else}
	<div class="space-y-6">
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
			<div>
				...
				<p class="font-mono text-sm text-muted-foreground">
					Action items and business intelligence
				</p>
			</div>
		</div>
		...
		<div class="border-t pt-4">
			<button
				class="flex items-center gap-2 font-mono text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
			...
			>
				<svg
					...
					stroke-width="1.5"
					stroke="currentColor"
					class="h-4 w-4 transition-transform {deepDiveOpen ? 'rotate-90' : ''}"
				>
		...
		{#if deepDiveOpen}
			<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				...
				{#if activeTab === 'deliveries' && availableYears.length > 0}
					<div class="flex items-center gap-2">
						...
						<select
							id="year-select"
							class="cursor-pointer rounded-md border bg-background px-3 py-1.5 text-sm shadow-sm focus:ring-2 focus:ring-ring focus:outline-none"
							value={selectedYear ?? ''}
			...
			<div class="flex gap-1 rounded-lg border bg-muted/40 p-1">
				<button
					class="cursor-pointer rounded-md px-4 py-1.5 text-sm font-medium transition-colors {activeTab ===
					'deliveries'
						? 'bg-background text-foreground shadow-sm'
						: 'text-muted-foreground hover:text-foreground'}"
			...
			{#if activeTab === 'shows'}
				...
				<div class="flex flex-wrap items-center gap-3">
					<select
						class="h-9 min-w-[300px] cursor-pointer rounded-lg border border-input bg-background px-3 text-sm text-foreground shadow-sm focus:ring-2 focus:ring-ring focus:outline-none"
					...
					{#if showDates.length === 0}
						<div class="rounded-none p-12 text-center">
							...
							<p class="mt-2 text-sm text-muted-foreground">
								Add shows in Organization settings to start tracking market events
							</p>
						</div>
					{:else}
										...
										>
											<td class="px-4 py-3">
												...
												<p class="mt-0.5 text-xs text-muted-foreground">
													<span
														class="mr-1 inline-flex items-center rounded bg-muted px-1.5 py-0.5 text-[11px] font-medium"
														>{monthNames[sd.month - 1]} {sd.year}</span
													>{[sd.city, sd.state].filter(Boolean).join(', ')}
												</p>
											</td>
											<td class="px-4 py-3 text-right text-sm">{summary?.appointments ?? 0}</td>
											<td class="px-4 py-3 text-right text-sm">{summary?.orders ?? 0}</td>
											<td class="px-4 py-3 text-right font-mono text-sm"
												>{formatCurrencyShort(summary?.revenue ?? 0)}</td
											>
										</tr>
									{/each}
								</tbody>
				...
				{:else}
					<!-- Show visits grid -->
					{@const freeformAppts = showAppointments.filter(
						(a) => !a.account_id && a.freeform_account_name
					)}
						...
						<div class="overflow-x-auto rounded-none border">
							<table class="w-full">
								<thead>
									<tr class="border-b bg-muted/40">
										<th
											class="sticky left-0 z-10 bg-muted/40 px-4 py-2 text-left text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
										>
											Account
										</th>
										<th
											class="px-3 py-2 text-left text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
										>
											Buyer
										</th>
										{#if isAdmin}
										...
										{/each}
										<th
											class="px-3 py-2 text-center text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
										>
											Notes
										</th>
									</tr>
									...
									{#each showVisits as visit (visit.id)}
										...
										<tr class="border-b transition-colors last:border-b-0 hover:bg-muted/20">
													...
													{#if repName}
														<TooltipProvider>
															<Tooltip>
																<TooltipTrigger>
																	<div
																		class="flex h-6 w-6 items-center justify-center rounded-full bg-muted text-[10px] font-medium"
																	>
																		{repName
																			.split(' ')
																			.map((w: string) => w[0])
											...
											{#each activeDeliveryColumns as col (col.id)}
												...
												>
													{#if val !== null}
														<div class="font-mono text-sm">{formatCurrencyShort(val)}</div>
													{:else}
												...
												{#if visit.notes}
													<div class="relative inline-block">
														...
														{#if hoveredNoteId === visit.id}
															<div
																class="absolute top-full right-0 z-20 mt-1 w-64 rounded-lg border bg-popover p-3 text-left text-sm text-popover-foreground shadow-md"
															>
																{visit.notes}
															</div>
														{/if}
					...
					<div class="overflow-x-auto rounded-none border">
						<table class="w-full">
							<thead>
								<tr class="border-b bg-muted/40">
									<th
										class="px-4 py-2 text-left text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
										>Order #</th
									>
									<th
										class="px-3 py-2 text-left text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
									...
									>
									<th
										class="px-3 py-2 text-right text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
										>Ordered</th
									>
									<th
										class="px-3 py-2 text-right text-[12px] font-medium tracking-wider text-muted-foreground uppercase"
								...
								{#each commissionRows as row (row.id)}
									<tr class="border-b transition-colors last:border-b-0 hover:bg-muted/20">
										...
										<td class="px-3 py-3 text-right font-mono text-sm"
											>{formatCurrency(row.total_amount)}</td
										>
										<td class="px-3 py-3 text-right font-mono text-sm"
											>{formatCurrency(row.shipped_amount ?? row.total_amount)}</td
										>
										<td class="px-3 py-3 text-right font-mono text-sm"
											>{fmtRate(row.effectiveRate)}</td
										>
										<td class="px-3 py-3 text-right font-mono text-sm"
											>{formatCurrency(row.commissionAmount)}</td
										>
									</tr>
								{/each}
							</tbody>
								...
								<tbody>
									{#each commissionMonthly as row (row.sortKey)}
										<tr class="border-b transition-colors last:border-b-0 hover:bg-muted/20">
											...
											<td class="px-3 py-3 text-right font-mono text-sm"
												>{formatCurrency(row.shippedTotal)}</td
											>
											<td class="px-3 py-3 text-right font-mono text-sm"
												>{formatCurrency(row.commissionTotal)}</td
											>
										</tr>
									{/each}
								</tbody>
			...
			{:else if activeTab === 'velocity'}
				{@const maxAccounts =
					styleVelocity.length > 0 ? Math.max(...styleVelocity.map((s) => s.account_count)) : 1}
							...
							<tbody>
								{#each styleVelocity as style (`${style.brand_name}::${style.style_number}`)}
									<tr class="border-b transition-colors last:border-b-0 hover:bg-muted/20">
										...
										<td class="px-3 py-3">
											<div class="flex items-center gap-2">
												<div class="max-w-[120px] min-w-[60px] flex-1">
													<div
												...
												<span
													class="text-sm font-semibold tabular-nums {style.account_count >= 10
										...
										<td class="px-3 py-3 text-right text-sm tabular-nums"
											>{formatNumber(style.total_qty)}</td
										>
										<td class="px-3 py-3 text-right font-mono text-sm tabular-nums"
											>{formatCurrencyShort(style.total_revenue)}</td
										>
										<td class="px-3 py-3 text-right text-sm tabular-nums"
											>{style.avg_qty_per_account}</td
										>
										<td class="px-3 py-3">
			...
			{:else if activeTab === 'deliveries'}
				{#if deliveries.length === 0}
					<Card>
						...
						<CardContent>
							<p class="text-sm text-muted-foreground">
								Once you have delivery dates configured and orders assigned to them, this grid will
								show revenue by account and delivery date.
							</p>
						</CardContent>
					</Card>
				{:else if activeGridAccounts.length === 0}
					<Card>
						<CardHeader>
							...
							<CardDescription>Add active accounts to see the delivery grid.</CardDescription>
						</CardHeader>
					</Card>
				{:else}
									...
									<tr class="border-b transition-colors last:border-b-0 hover:bg-muted/20">
										<td class="sticky left-0 z-10 min-w-[200px] bg-background px-4 py-3">
											<div class="flex items-center gap-2">
												...
												{#if account.notes}
													<div class="relative">
														<button
															class="ml-1 cursor-pointer rounded p-0.5 text-muted-foreground/50 transition-colors hover:text-muted-foreground"
														...
														{#if hoveredNoteId === account.id}
															<div
																class="absolute top-full left-0 z-20 mt-1 w-64 rounded-lg border bg-popover p-3 text-sm text-popover-foreground shadow-md"
															>
																{account.notes}
															</div>
														{/if}
										...
										{#each deliveries as delivery (delivery.id)}
											...
											<td class="border-l border-border/50 px-3 py-3 text-right whitespace-nowrap">
												{#if current !== null}
													<div class="font-mono text-sm">{formatCurrencyShort(current)}</div>
												{:else}
													<div class="text-sm text-muted-foreground/40">&mdash;</div>
												{/if}
												{#if prior !== null}
													<div class="font-mono text-xs text-muted-foreground">
														{formatCurrencyShort(prior)}
													</div>
												{:else}
