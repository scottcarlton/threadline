<script lang="ts">
	...
	import ConnectionCard from '$lib/components/settings/ConnectionCard.svelte';
	...
	const anyEmailConnected = $derived(data.emailConnected || data.outlookConnected);
	const anyCalendarConnected = $derived(
		data.calendarConnected || data.msCalendarConnected || data.calendlyConnected
	);
	const connectLinkClass =
		'text-sm font-medium text-foreground transition-colors hover:text-muted-foreground';
			...
			<div class="mt-4">
					...
					<div class="space-y-3">
						...
						{#if data.outlookConnected}
							<ConnectionCard
								provider="outlook"
								name="Outlook"
								detail={data.outlookEmail ?? ''}
								onDisconnect={handleDisconnectOutlook}
								disconnecting={disconnectingOutlook}
							/>
						{/if}
						{#if !data.emailConnected || !data.outlookConnected}
							<div class="flex flex-wrap gap-x-6 gap-y-2 pt-1">
								{#if !data.emailConnected}
									<a href={resolve('/api/email/connect')} class={connectLinkClass}
										>+ Connect Gmail instead</a
									>
								{/if}
								{#if !data.outlookConnected}
									<a href={resolve('/api/email-outlook/connect')} class={connectLinkClass}
										>+ Connect Outlook instead</a
									>
								{/if}
							</div>
						{/if}
				...
				{:else}
					<div class="flex gap-4">
						...
						>
							<MailProviderLogo provider="gmail" cls="h-4 w-4" />
							Gmail
						</Button>
						<Button
							...
							class="w-full sm:w-auto sm:min-w-[200px]"
						>
							<MailProviderLogo provider="outlook" cls="h-5 w-5" />
							Outlook
						</Button>
					</div>
				{/if}
					...
					<div class="space-y-3">
						{#if data.calendarConnected}
							<ConnectionCard
								...
								detail={data.calendarEmail ?? ''}
								onDisconnect={handleDisconnectCalendar}
								disconnecting={disconnectingCalendar}
							/>
						{/if}
						{#if data.msCalendarConnected}
							<ConnectionCard
								...
								detail={data.msCalendarEmail ?? ''}
								onDisconnect={handleDisconnectMsCalendar}
								disconnecting={disconnectingMsCalendar}
							/>
						{/if}
						{#if data.calendlyConnected}
							<ConnectionCard
								provider="calendly"
								name="Calendly"
								detail={data.calendlyEmail ?? ''}
								onDisconnect={handleDisconnectCalendly}
								disconnecting={disconnectingCalendly}
							/>
						{/if}
						{#if !data.calendarConnected || !data.msCalendarConnected || !data.calendlyConnected}
							<div class="flex flex-wrap gap-x-6 gap-y-2 pt-1">
								{#if !data.calendarConnected}
									<a
										href={resolve('/api/integrations/google-calendar/connect')}
										class={connectLinkClass}>+ Google Calendar</a
									>
								{/if}
								{#if !data.msCalendarConnected}
									<a
										href={resolve('/api/integrations/microsoft-calendar/connect')}
										class={connectLinkClass}>+ Microsoft Calendar</a
									>
								{/if}
								{#if !data.calendlyConnected}
									<a href={resolve('/api/integrations/calendly/connect')} class={connectLinkClass}
										>+ Calendly</a
									>
								{/if}
							</div>
