<script lang="ts">
	...
	const expenseSummary = $derived(
		data.expenseSummary as { total: number; pendingCount: number; pendingAmount: number }
	);
			...
			<Card>
				<CardContent class="pt-4 pb-4">
					...
					{#if performance.yoyGrowth !== 0}
						<p
							class="mt-0.5 text-sm {performance.yoyGrowth > 0
								? 'text-emerald-600'
								: 'text-red-600'}"
						>
							{performance.yoyGrowth > 0 ? '+' : ''}{Math.round(performance.yoyGrowth)}% vs last
							year
						</p>
					{/if}
				</CardContent>
			...
			<Card>
				<CardContent class="pt-4 pb-4">
					...
					<p class="mt-0.5 text-sm text-muted-foreground">
						{fmt.format(performance.avgOrderValue)} avg
					</p>
				</CardContent>
			</Card>
			<Card>
				<CardContent class="pt-4 pb-4">
					...
					<p class="mt-0.5 text-sm text-muted-foreground">
						{performance.totalOrders} lifetime orders
					</p>
				</CardContent>
			</Card>
			<Card>
				<CardContent class="pt-4 pb-4">
					...
					{#if performance.ytdRevenue > 0}
						<p class="mt-0.5 text-sm text-muted-foreground">
							{Math.round((performance.ytdShipped / performance.ytdRevenue) * 100)}% of booked
						</p>
					{/if}
				</CardContent>
			</Card>
					...
					<div class="space-y-2">
						{#each performance.topAccounts as acct, i}
							...
							>
								<div class="flex items-center gap-3">
									<span
										class="flex h-6 w-6 items-center justify-center rounded-full bg-muted text-sm font-medium text-muted-foreground"
										>{i + 1}</span
									>
									<span class="text-sm font-medium">{acct.name}</span>
								</div>
								<span class="font-mono text-sm font-medium">{fmt.format(acct.total)}</span>
							</a>
						{/each}
					</div>
				</CardContent>
							...
							<div>
								...
								<p class="text-sm text-muted-foreground">
									{#if expenseSummary.pendingCount > 0}
										{expenseSummary.pendingCount} pending ({fmt.format(
											expenseSummary.pendingAmount
										)})
						...
						<div class="divide-y rounded-none border">
							{#each brandAssets as asset}
								<div class="flex items-center gap-3 px-4 py-3">
									...
									<!-- Category badge -->
									<span
										class="inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-xs font-medium {categoryColors[
											asset.category
