<script lang="ts">
	...
	async function handleShowImport(rows: Record<string, string>[]): Promise<{ success: number; errors: string[] }> {
		let success = 0;
		const errors: string[] = [];
		for (let i = 0; i < rows.length; i++) {
			const row = rows[i];
			if (!row.name?.trim()) { errors.push(`Row ${i + 1}: Show Name is required`); continue; }
	...
	async function openDrawer(showId: string) {
		if (drawerShowId === showId && drawerOpen) { closeDrawer(); return; }
		drawerShowId = showId;
		drawerOpen = true;
		drawerLoading = true;
		dEditing = false;
	...
	async function dHandleSave() {
		...
		dError = '';
		dLoading = true;
		const { error: err } = await supabase.from('shows').update({ name: dName, notes: dNotes || null, updated_at: new Date().toISOString() }).eq('id', drawerShow.id);
		dLoading = false;
		if (err) { dError = err.message; } else { dEditing = false; await invalidateAll(); await refreshDrawer(); }
	}
	...
	async function dSaveDate() {
		...
		dDateLoading = true;
		const { error: err } = await supabase.from('show_dates').insert({
			show_id: drawerShow.id, organization_id: data.organization?.id,
			year: dNewYear, month: dNewMonth, venue: dNewVenue || null, city: dNewCity || null, state: dNewState || null
		});
		dDateLoading = false;
		if (err) { dDateError = err.message; } else { dAddingDate = false; dNewMonth = 1; dNewYear = new Date().getFullYear(); dNewCity = ''; dNewState = ''; dNewVenue = ''; await invalidateAll(); await refreshDrawer(); }
	}
	...
	async function dDeleteDocument(dateId: string, documentId: string) {
		await fetch(`/api/shows/${dateId}/documents`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ documentId }) });
		await refreshDrawer();
	}
...
<div class="space-y-6">
	...
	{#if creatingShow}
		<div class="rounded-none border p-4 space-y-3">
			...
			{/if}
			<form onsubmit={(e) => { e.preventDefault(); createShow(); }} class="flex items-end gap-3">
				<div class="flex-1 space-y-1">
					<Label for="new-show-name">Name *</Label>
					<Input id="new-show-name" bind:value={newShowName} required placeholder="Brand Assembly" />
				</div>
	...
	{#if filtered.length === 0}
		<div class="rounded-none p-12 text-center">
			...
			{:else}
				...
				<p class="mt-2 text-sm text-muted-foreground">Add your first show to start tracking market events</p>
			{/if}
		</div>
	{:else}
		<div class="space-y-2">
			{#each filtered as show}
				<div class="rounded-none border overflow-hidden">
					<!-- Series header -->
					<div class="flex items-center justify-between px-4 py-3 bg-muted/30">
						<button
							type="button"
							class="flex items-center gap-2 text-sm font-medium cursor-pointer hover:underline"
							onclick={() => toggleExpand(show.id)}
						>
							<span class="text-muted-foreground">{expandedIds.has(show.id) ? '▾' : '▸'}</span>
							<span class="cursor-pointer text-base hover:underline" onclick={(e) => { e.stopPropagation(); openDrawer(show.id); }}>{show.name}</span>
							<span class="text-sm font-mono text-muted-foreground font-normal">({show.show_dates?.length ?? 0} Show{(show.show_dates?.length ?? 0) === 1 ? '' : 's'})</span>
						</button>
						{#if canEdit}
							<button
								type="button"
						...
						<div class="divide-y">
							{#each (show.show_dates ?? []) as date}
								<div class="flex items-center gap-6 px-6 py-2.5 text-sm">
									...
									<span class="font-mono text-muted-foreground">{[date.city, date.state].filter(Boolean).join(', ') || '—'}</span>
							...
							{#if addingDateForShowId === show.id}
								<div class="px-6 py-3 space-y-3 bg-muted/10">
									{#if dateError}
										<div class="rounded-md bg-destructive/10 p-2 text-sm text-destructive">{dateError}</div>
									{/if}
									<form onsubmit={(e) => { e.preventDefault(); saveDate(); }} class="flex flex-wrap items-end gap-3">
										<div class="space-y-1">
										...
										<div class="space-y-1">
											...
											<Input id="date-year" type="number" bind:value={newDateYear} min={2020} max={2040} class="w-24" />
										</div>
										<div class="space-y-1">
											<Label for="date-city">City</Label>
											<Input id="date-city" bind:value={newDateCity} placeholder="New York" class="w-36" />
										</div>
										<div class="space-y-1">
											<Label for="date-state">State</Label>
											<Input id="date-state" bind:value={newDateState} placeholder="NY" class="w-20" />
										</div>
										<div class="space-y-1">
											<Label for="date-venue">Venue</Label>
											<Input id="date-venue" bind:value={newDateVenue} placeholder="Convention Center" class="w-44" />
										</div>
										<Button type="submit" size="sm" disabled={dateLoading}>
											{dateLoading ? 'Saving...' : 'Save'}
										</Button>
										<Button variant="outline" size="sm" onclick={() => (addingDateForShowId = null)}>Cancel</Button>
									</form>
								</div>
							{/if}
						</div>
			...
			{:else if drawerShow}
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						...
						{#if canEdit && !dEditing}
							<button class="text-sm text-muted-foreground hover:text-foreground" onclick={dStartEdit}>Edit</button>
						{/if}
					</div>
					...
					{#if dEditing}
						<form onsubmit={(e) => { e.preventDefault(); dHandleSave(); }} class="space-y-3">
							<div class="space-y-1">
							...
							<div class="space-y-1">
								...
								<textarea id="d-notes" bind:value={dNotes} rows="2" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm"></textarea>
							</div>
							<div class="flex gap-2">
								<Button type="submit" size="sm" disabled={dLoading}>{dLoading ? 'Saving...' : 'Save'}</Button>
								<Button variant="outline" size="sm" onclick={() => (dEditing = false)}>Cancel</Button>
							</div>
						</form>
					{:else}
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						...
						{#if canEdit && !dAddingDate}
							<button class="text-sm text-muted-foreground hover:text-foreground" onclick={() => (dAddingDate = true)}>+ Add</button>
						{/if}
					</div>
							...
							<div class="flex items-center justify-between">
								<div class="text-sm">
									...
									{#if date.city || date.state}
										<span class="font-mono text-muted-foreground ml-2">{[date.city, date.state].filter(Boolean).join(', ')}</span>
									{/if}
									{#if date.venue}
										<span class="font-mono text-muted-foreground ml-2">{date.venue}</span>
									{/if}
								</div>
								{#if canEdit}
									<button class="text-xs text-muted-foreground hover:text-destructive" onclick={() => dDeleteDate(date.id)}>Remove</button>
								{/if}
							</div>
								...
								<div class="border-t pt-2 space-y-1">
									{#each drawerDocuments[date.id] ?? [] as doc}
										<div class="flex items-center justify-between text-xs">
											<span class="text-muted-foreground">{doc.name} {doc.file_size ? formatFileSize(doc.file_size) : ''}</span>
											{#if canEdit}
												<button class="text-muted-foreground hover:text-destructive" onclick={() => dDeleteDocument(date.id, doc.id)}>Remove</button>
											{/if}
										</div>
									{/each}
									{#if canEdit}
										<label class="inline-flex cursor-pointer items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground">
											{#if dUploadingForDateId === date.id}
												Uploading...
											{:else}
												+ Attach file
											{/if}
											<input type="file" class="hidden" onchange={(e) => { const t = e.target as HTMLInputElement; const f = t.files?.[0]; if (f) dUploadDocument(date.id, f); t.value = ''; }} />
										</label>
									{/if}
								</div>
							{/if}
					...
					{#if dAddingDate}
						{#if dDateError}
							<div class="rounded-md bg-destructive/10 p-2 text-xs text-destructive">{dDateError}</div>
						{/if}
						<form onsubmit={(e) => { e.preventDefault(); dSaveDate(); }} class="space-y-2 rounded-lg border border-dashed p-3">
							<div class="flex flex-wrap gap-2">
								<div class="space-y-1">
									<Label for="d-month">Month</Label>
									<select id="d-month" bind:value={dNewMonth} class="flex h-9 w-20 rounded-md border border-input bg-background px-2 text-sm" data-size="sm">
										{#each monthNames as m, i}
											<option value={i + 1}>{m}</option>
										{/each}
									</select>
								...
								<div class="space-y-1">
									...
									<Input id="d-year" type="number" bind:value={dNewYear} min={2020} max={2040} class="w-20" />
								</div>
								<div class="space-y-1">
							...
							<div class="flex gap-2">
								<Button type="submit" size="sm" disabled={dDateLoading}>{dDateLoading ? 'Saving...' : 'Save'}</Button>
								<Button variant="outline" size="sm" onclick={() => (dAddingDate = false)}>Cancel</Button>
							</div>
						</form>
					{/if}
				</div>
