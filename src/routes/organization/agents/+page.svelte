...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="mt-0.5 text-sm text-muted-foreground">Custom AI agents to automate and enhance your workflows</p>
		</div>
		{#if isAdmin}
	...
	{#if agents.length === 0}
		<div class="rounded-none p-12 text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">Create custom AI agents with specialized prompts to automate tasks and enhance your sales workflows</p>
		</div>
	{:else}
					...
					<Card class="transition-colors hover:bg-muted/30">
						<CardContent class="pt-5 pb-5">
							<div class="flex items-start justify-between">
								<div class="flex items-center gap-3">
									...
									<div>
										...
										<p class="text-sm font-mono text-muted-foreground">@{agent.slug}</p>
									</div>
								</div>
							...
							{#if agent.description}
								<p class="mt-3 text-sm text-muted-foreground line-clamp-2">{agent.description}</p>
							{/if}
							<p class="mt-3 text-sm text-muted-foreground">
								Created by {agent.profiles?.display_name ?? 'Unknown'} · {new Date(agent.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
							</p>
						</CardContent>
					</Card>
				</a>
