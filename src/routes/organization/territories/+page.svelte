<script lang="ts">
	...
	async function handleTerritoryImport(rows: Record<string, string>[]): Promise<{ success: number; errors: string[] }> {
		let success = 0;
		const errors: string[] = [];
		for (let i = 0; i < rows.length; i++) {
			const row = rows[i];
			if (!row.name?.trim()) { errors.push(`Row ${i + 1}: Territory Name is required`); continue; }
	...
	const territories = $derived(data.territories as Array<{
		id: string;
		name: string;
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="mt-0.5 text-sm text-muted-foreground">Geographic regions with assigned accounts and reps</p>
		</div>
		{#if !adding}
	...
	{#if adding}
		<Card>
			...
			<CardContent>
				...
				{/if}
				<form onsubmit={(e) => { e.preventDefault(); createTerritory(); }} class="space-y-4">
	...
	{#if territories.length === 0 && !adding}
		<div class="rounded-none p-12 text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">Create territories to group accounts by region</p>
		</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b bg-muted/40">
						<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Territory</th>
						<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Assigned Rep</th>
						<th class="px-4 py-2.5 text-right text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Accounts</th>
					</tr>
				</thead>
