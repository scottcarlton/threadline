<script lang="ts">
	...
	async function openDrawer(contactId: string) {
		...
		if (contactId.startsWith(accountPrefix)) {
			...
			const [accRes, ordersRes] = await Promise.all([
				supabase.from('accounts').select('id, business_name, contact_first_name, contact_last_name, contact_email, phone, city, state').eq('id', accId).single(),
				supabase.from('orders').select('id, order_number, total_amount, status, order_year, brands(name)').eq('account_id', accId).order('created_at', { ascending: false }).limit(10)
			]);
			if (accRes.data) {
				const a = accRes.data;
				drawerContact = {
					name: [a.contact_first_name, a.contact_last_name].filter(Boolean).join(' ') || null,
					email: a.contact_email, phone: a.phone,
					source: 'account', sourceId: a.id, sourceName: a.business_name,
			...
			const [brRes, ordersRes] = await Promise.all([
				supabase.from('brands').select('id, name, contact_first_name, contact_last_name, contact_email, contact_phone, website').eq('id', brId).single(),
				supabase.from('orders').select('id, order_number, total_amount, status, order_year, accounts(business_name)').eq('brand_id', brId).order('created_at', { ascending: false }).limit(10)
			]);
			if (brRes.data) {
				const b = brRes.data;
				drawerContact = {
					...
					source: 'brand', sourceId: b.id, sourceName: b.name,
					website: b.website, location: null
				};
			}
			drawerOrders = ordersRes.data ?? [];
		} else {
			const { data: disc } = await supabase.from('discovered_contacts').select('*').eq('id', contactId).single();
			if (disc) {
				drawerContact = {
					name: disc.name, email: disc.email, phone: null,
					source: 'discovered', sourceId: disc.id, sourceName: null,
					status: disc.status, messageCount: disc.message_count,
					firstSeenAt: disc.first_seen_at, lastSeenAt: disc.last_seen_at
				};
			}
			drawerOrders = [];
		}
	...
	const drawerSourceLabel = $derived(
		drawerContact?.source === 'account' ? 'Account' : drawerContact?.source === 'brand' ? 'Brand' : 'Discovered'
	);

	const drawerSourceHref = $derived(
		drawerContact?.source === 'account' ? `/accounts/${drawerContact.sourceId}`
		: drawerContact?.source === 'brand' ? `/brands/${drawerContact.sourceId}`
		: null
	);
</script>
...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="mt-0.5 text-sm text-muted-foreground">Account and brand contacts across your organization</p>
		</div>
	...
	<div class="flex items-center gap-3">
		...
		<div class="flex gap-1 rounded-lg bg-muted p-1">
			<button
				class="rounded-md px-4 py-2 text-sm font-medium transition-colors {activeTab === 'known' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}"
				onclick={() => (activeTab = 'known')}
			>
				Known
			</button>
			<button
				class="rounded-md px-4 py-2 text-sm font-medium transition-colors {activeTab === 'discovered' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}"
				onclick={() => (activeTab = 'discovered')}
			>
				Discovered
				{#if discoveredContacts.length > 0}
					<span class="ml-1.5 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-zinc-100 px-1.5 text-[11px] font-medium text-zinc-600">{discoveredContacts.length}</span>
				{/if}
			</button>
		</div>
	</div>
	...
	{#if activeTab === 'known'}
		{#if filteredKnown.length === 0}
			<div class="rounded-none p-12 text-center">
				...
				{/if}
				<p class="mt-4 text-lg font-semibold">{search ? 'No contacts match your search' : 'Your contacts live here'}</p>
				<p class="mt-2 text-sm text-muted-foreground">{search ? 'Try adjusting your search terms' : 'Add contacts to your accounts or brands to see them here'}</p>
			</div>
		{:else}
				...
				<table class="w-full">
					<thead>
						<tr class="border-b bg-muted/40">
							<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Contact</th>
							<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Phone</th>
							<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Source</th>
						</tr>
					</thead>
					<tbody class="divide-y">
						{#each filteredKnown as contact}
							<tr
								class="transition-colors hover:bg-muted/30 cursor-pointer {drawerContactId === `${contact.source}-${contact.sourceId}` ? 'bg-muted/30' : ''}"
								onclick={() => openDrawer(`${contact.source}-${contact.sourceId}`)}
							>
								<td class="px-4 py-3">
									<span class="text-base font-medium">{contact.name ?? '—'}</span>
									<p class="text-sm font-mono text-muted-foreground">{contact.email}</p>
								</td>
								<td class="px-4 py-3">
									<span class="text-sm font-mono text-muted-foreground">{contact.phone ?? '—'}</span>
								</td>
								<td class="px-4 py-3">
									<div class="flex items-center gap-2">
	...
	{#if activeTab === 'discovered'}
		{#if filteredDiscovered.length === 0}
			<div class="rounded-none p-12 text-center">
				...
				{/if}
				<p class="mt-4 text-lg font-semibold">{search ? 'No discovered contacts match your search' : 'Discover contacts from your inbox'}</p>
				<p class="mt-2 text-sm text-muted-foreground">{search ? 'Try adjusting your search terms' : 'Click "Scan Inbox" to find contacts from your email'}</p>
			</div>
		{:else}
			<div class="overflow-hidden rounded-none border">
				<table class="w-full">
					<thead>
						<tr class="border-b bg-muted/40">
							<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Contact</th>
							<th class="px-4 py-2.5 text-right text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Times Seen</th>
							<th class="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Last Seen</th>
							<th class="px-4 py-2.5 text-right text-[12px] font-medium uppercase tracking-wider text-muted-foreground">Actions</th>
						</tr>
					</thead>
					<tbody class="divide-y">
						{#each filteredDiscovered as contact}
							<tr
								class="transition-colors hover:bg-muted/30 cursor-pointer {drawerContactId === contact.id ? 'bg-muted/30' : ''}"
							...
							>
								...
								<td class="px-4 py-3">
									<span class="text-sm text-muted-foreground">{formatDate(contact.last_seen_at)}</span>
								</td>
								...
								<td class="px-4 py-3 text-right" onclick={(e) => e.stopPropagation()}>
									<div class="flex items-center justify-end gap-1">
										{#if contact.status === 'new'}
											<Button variant="outline" size="sm" disabled={updatingId === contact.id} onclick={() => updateStatus(contact.id, 'saved')}>Save</Button>
											<Button variant="ghost" size="sm" disabled={updatingId === contact.id} onclick={() => updateStatus(contact.id, 'dismissed')}>Dismiss</Button>
		...
		<div class="flex items-center justify-between px-5 py-4">
			{#if drawerContact}
				<div class="flex items-center gap-3 min-w-0">
					...
					<div class="min-w-0">
						<h2 class="text-base font-semibold truncate">{drawerContact.name ?? drawerContact.email ?? 'Unknown'}</h2>
						{#if drawerContact.name && drawerContact.email}
							<p class="text-sm font-mono text-muted-foreground truncate">{drawerContact.email}</p>
						{/if}
					</div>
				</div>
			{:else}
			...
			{/if}
			<button onclick={closeDrawer} class="rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground shrink-0">
					...
					<dl class="space-y-2.5 text-sm">
						{#if drawerContact.email}
							<div class="flex justify-between">
								...
								<dd><a href="mailto:{drawerContact.email}" class="font-mono hover:underline">{drawerContact.email}</a></dd>
							</div>
						{/if}
						...
						{#if drawerContact.website}
							<div class="flex justify-between">
								...
								<dd><a href={drawerContact.website} target="_blank" rel="noopener noreferrer" class="text-primary hover:underline">{drawerContact.website.replace(/^https?:\/\//, '')}</a></dd>
							</div>
						{/if}
						{#if drawerContact.sourceName && drawerSourceHref}
							<div class="flex justify-between border-t pt-2.5">
								...
								<dd><a href={drawerSourceHref} class="font-medium hover:underline">{drawerContact.sourceName}</a></dd>
							</div>
						{/if}
					...
					<div class="space-y-3">
						...
						<div class="space-y-1.5">
							{#each drawerOrders as order}
								<a href="/orders/{order.id}" class="flex items-center justify-between rounded-lg border px-3 py-2 hover:bg-muted/30 transition-colors">
									<div>
										<span class="text-sm font-medium">{order.order_number}</span>
										<p class="text-xs text-muted-foreground">
											{drawerContact?.source === 'brand' ? ((order.accounts as any)?.business_name ?? '') : ((order.brands as any)?.name ?? '')}
										</p>
									</div>
