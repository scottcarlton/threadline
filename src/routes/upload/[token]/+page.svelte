	...
	<div class="w-full max-w-sm space-y-6 text-center">
			...
			<div class="space-y-3">
				<Button class="w-full" size="lg" onclick={() => cameraInput?.click()} disabled={uploading}>
					{#if uploading}
						Uploading...
					{:else}
						<svg xmlns="http://www.w3.org/2000/svg" class="-ml-1 mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				<Button class="w-full" variant="outline" size="lg" onclick={() => libraryInput?.click()} disabled={uploading}>
					Choose from Library
				</Button>
			</div>
		...
		{#if uploaded.length > 0}
			<div class="space-y-1.5">
				<p class="text-sm font-medium">{uploaded.length} receipt{uploaded.length !== 1 ? 's' : ''} uploaded</p>
