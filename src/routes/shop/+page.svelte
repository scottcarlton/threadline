...
<div class="space-y-6">
	<div>
		...
		{#if brands.length > 0}
			<p class="mt-1 text-sm text-muted-foreground">
				{filtered.length} product{filtered.length !== 1 ? 's' : ''}{activeBrandName ? ` from ${activeBrandName}` : ''}
			</p>
		{/if}
	</div>
	...
	{#if brands.length === 0}
		<div class="rounded-none p-12 text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">You currently do not have access to any brand items. Contact your sales rep for access.</p>
		</div>
	{:else}
	<!-- Filters -->
	<div class="flex flex-wrap items-center gap-3">
		...
		<div class="flex flex-wrap gap-1.5">
			<button
				class="rounded-lg border px-3 py-1.5 text-sm transition-colors {!brandFilter ? 'border-primary bg-primary text-primary-foreground' : 'text-muted-foreground hover:border-foreground/20 hover:text-foreground'}"
			...
			{#each brands as brand}
				<button
					class="rounded-lg border px-3 py-1.5 text-sm transition-colors {brandFilter === brand.id ? 'border-primary bg-primary text-primary-foreground' : 'text-muted-foreground hover:border-foreground/20 hover:text-foreground'}"
	...
	{#if filtered.length === 0}
		<div class="rounded-none p-12 text-center">
			...
			{:else}
				...
				<p class="mt-2 text-sm text-muted-foreground">Products will appear here once your brands add them</p>
			{/if}
		</div>
	{:else}
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each filtered as product}
				...
				<div class="group rounded-none border bg-card transition-all duration-200 hover:border-foreground/20 hover:shadow-md">
					<a href="/shop/{product.id}" class="block">
						...
						<div class="p-4 pb-2">
							{#if !brandFilter}
								<p class="text-xs font-medium uppercase tracking-wider text-muted-foreground">{product.brands?.name}</p>
							{/if}
							<p class="text-sm text-muted-foreground">{product.style_number}</p>
							<p class="mt-0.5 text-base font-normal">{product.name}</p>
							<div class="mt-2 flex items-center justify-between">
								<span class="text-base font-normal">{fmt.format(Number(product.wholesale_price))}</span>
								<span class="text-sm text-muted-foreground">{getVariantSummary(product.product_variants ?? [])}</span>
							</div>
							{#if product.category}
								<span class="mt-2 inline-flex rounded-md bg-muted px-2 py-0.5 text-xs text-muted-foreground">{product.category}</span>
							{/if}
						</div>
					</a>
					<div class="px-4 pb-4">
						...
						{:else}
							<button
								onclick={() => cart.addItem({
									...
									colors: [...new Set((product.product_variants ?? []).map((v: any) => v.color).filter(Boolean))],
									sizes: [...new Set((product.product_variants ?? []).map((v: any) => v.size).filter(Boolean))],
