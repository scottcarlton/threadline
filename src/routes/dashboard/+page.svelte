					...
					{#each buyerData.recentOrders as order (order.id)}
						...
						>
							...
							<div class="flex items-center gap-3">
								...
								>
								<span
									class="rounded-full bg-zinc-500/10 px-2 py-0.5 text-sm text-zinc-400 capitalize"
									>{order.status}</span
								>
							</div>
						</a>
	...
	{:else}
		<div class="text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">
				When a brand connects with you, their catalog and your orders appear here. Share your store
				details with the brands you buy from.
			</p>
		</div>
	{/if}
</div>
