...
<div class="space-y-6">
	...
	{#if rows.length === 0}
		<div class="rounded-none p-12 text-center">
			...
			<p class="mt-2 text-sm text-muted-foreground">Data will appear here once orders come in{report !== 'pipeline' ? ` for ${year}` : ''}</p>
		</div>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b bg-muted/40">
						<th class="px-4 py-2.5 text-left text-sm font-medium">{report === 'sales-by-brand' ? 'Brand' : report === 'sales-by-account' ? 'Account' : 'Rep'}</th>
				...
				<tbody class="divide-y">
					{#each rows as row}
						<tr class="hover:bg-muted/30">
							...
							<td class="px-4 py-3 text-right text-sm font-mono text-muted-foreground">{fmt.format(row.orders > 0 ? row.revenue / row.orders : 0)}</td>
							<td class="px-4 py-3 text-right text-sm font-mono">{fmt.format(row.revenue)}</td>
						</tr>
					{/each}
				</tbody>
				<tfoot>
					<tr class="bg-muted/40">
						...
						<td class="px-4 py-2.5 text-right text-sm font-medium">{rows.reduce((s, r) => s + r.orders, 0)}</td>
						<td class="px-4 py-2.5"></td>
						<td class="px-4 py-2.5 text-right text-sm font-mono font-bold">{fmt.format(rows.reduce((s, r) => s + r.revenue, 0))}</td>
					</tr>
				</tfoot>
			</table>
		</div>
				...
				<tbody class="divide-y">
					{#each rows as row}
						<tr class="hover:bg-muted/30">
							...
							<td class="px-4 py-3 text-right text-sm font-mono">{fmt.format(row.orderAmount)}</td>
							<td class="px-4 py-3 text-right text-sm text-muted-foreground">{row.brandRate}%</td>
							<td class="px-4 py-3 text-right text-sm font-mono">{fmt.format(row.brandCommission)}</td>
							<td class="px-4 py-3 text-sm">{row.rep}</td>
							<td class="px-4 py-3 text-right text-sm text-muted-foreground">{row.repRate}%</td>
							<td class="px-4 py-3 text-right text-sm font-mono">{fmt.format(row.repCommission)}</td>
						</tr>
					{/each}
				</tbody>
				<tfoot>
					<tr class="bg-muted/40">
						...
						<td class="px-4 py-2.5 text-right text-sm font-mono font-bold">{fmt.format(rows.reduce((s, r) => s + r.orderAmount, 0))}</td>
						<td class="px-4 py-2.5"></td>
						<td class="px-4 py-2.5 text-right text-sm font-mono font-bold">{fmt.format(rows.reduce((s, r) => s + r.brandCommission, 0))}</td>
						<td class="px-4 py-2.5"></td>
						<td class="px-4 py-2.5"></td>
						<td class="px-4 py-2.5 text-right text-sm font-mono font-bold">{fmt.format(rows.reduce((s, r) => s + r.repCommission, 0))}</td>
					</tr>
				</tfoot>
			</table>
		</div>
				...
				<tbody class="divide-y">
					{#each rows as row}
						<tr class="hover:bg-muted/30">
							...
							<td class="px-4 py-3 text-right text-sm font-mono">{fmt.format(Number(row.total_amount))}</td>
						</tr>
					{/each}
				</tbody>
			</table>
