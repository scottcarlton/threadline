...
export const load: PageServerLoad = async ({ locals, params, url }) => {
	...
	if (!organization) return { report, title: reportTitles[report], year: new Date().getFullYear(), rows: [] };
	...
	switch (report) {
		case 'sales-by-brand': {
			const { data: orders } = await scopeByRep(scopeByBrand(supabase
				.from('orders')
			...
			for (const o of orders ?? []) {
				...
				if (existing) { existing.orders++; existing.revenue += Number(o.total_amount); }
				else brands.set(o.brand_id, { name, orders: 1, revenue: Number(o.total_amount) });
			}
			return { report, title: reportTitles[report], year, rows: Array.from(brands.values()).sort((a, b) => b.revenue - a.revenue) };
		}
		...
		case 'sales-by-account': {
			...
			return { report, title: reportTitles[report], year, rows: Array.from(accounts.values()).sort((a, b) => b.revenue - a.revenue) };
		}
		...
		case 'sales-by-territory': {
			...
			const territories = new Map<string, { name: string; orders: number; revenue: number; accounts: Set<string> }>();
			for (const o of orders ?? []) {
				const terr = (o.accounts as any)?.territories?.name ?? 'Unassigned';
				const existing = territories.get(terr);
				if (existing) { existing.orders++; existing.revenue += Number(o.total_amount); existing.accounts.add(o.account_id); }
				else territories.set(terr, { name: terr, orders: 1, revenue: Number(o.total_amount), accounts: new Set([o.account_id]) });
			}
			return { report, title: reportTitles[report], year, rows: Array.from(territories.values()).map(t => ({ name: t.name, orders: t.orders, revenue: t.revenue, accountCount: t.accounts.size })).sort((a, b) => b.revenue - a.revenue) };
		}
		...
		case 'sales-by-rep': {
			...
			const nameMap = new Map((members ?? []).map((m: any) => [m.profile_id, m.profiles?.display_name ?? 'Unknown']));
			...
			return { report, title: reportTitles[report], year, rows: Array.from(reps.values()).sort((a, b) => b.revenue - a.revenue) };
		}
		...
		case 'commission': {
			...
			const memberToName = new Map((members ?? []).map((m: any) => [m.id, m.profiles?.display_name ?? 'Unknown']));
			const commMap = new Map((memberCommissions ?? []).map((mc: any) => [`${mc.member_id}-${mc.brand_id}`, mc.rate]));
			...
			const rows = (orders ?? []).map((o: any) => {
				...
				return {
					brand: o.brands?.name ?? 'Unknown', account: o.accounts?.business_name ?? 'Unknown',
					orderAmount: amount, brandRate, brandCommission: amount * brandRate / 100,
					rep: memberToName.get(memberId ?? '') ?? 'Unknown', repRate, repCommission: amount * repRate / 100
				};
		...
		case 'pipeline': {
			...
			for (const o of orders ?? []) {
				...
				if (existing) { existing.count++; existing.total_amount += Number(o.total_amount); }
				else statuses.set(o.status, { status: o.status, count: 1, total_amount: Number(o.total_amount) });
			}
		...
		case 'season-comparison': {
			...
			const seasons = new Map<string, { name: string; year: number; orders: number; revenue: number }>();
			for (const o of orders ?? []) {
				const key = `${o.season_id}-${o.order_year}`;
				const name = (o.seasons as any)?.name ?? 'Unknown';
				const existing = seasons.get(key);
				if (existing) { existing.orders++; existing.revenue += Number(o.total_amount); }
				else seasons.set(key, { name: `${name} ${o.order_year ?? ''}`.trim(), year: o.order_year ?? 0, orders: 1, revenue: Number(o.total_amount) });
			}
			return { report, title: reportTitles[report], year, rows: Array.from(seasons.values()).sort((a, b) => b.year - a.year || b.revenue - a.revenue) };
		}
		...
		case 'show-performance': {
			...
			const [ordersRes, visitsRes, apptsRes] = await Promise.all([
				scopeByRep(scopeByBrand(supabase.from('orders').select('show_date_id, total_amount').eq('organization_id', orgId).in('show_date_id', dateIds).neq('status', 'cancelled'))),
				supabase.from('show_visits').select('show_date_id, is_new_account').in('show_date_id', dateIds),
				supabase.from('appointments').select('show_date_id').in('show_date_id', dateIds)
			]);

			const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
			const rows = (showDates ?? []).map((sd: any) => {
				const showName = Array.isArray(sd.shows) ? sd.shows[0]?.name : sd.shows?.name ?? 'Unknown';
