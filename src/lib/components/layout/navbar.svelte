<script lang="ts">
	...
	let { user, organization, orgDisplayName, sidebarOpen, showTeam = true, role = null, isBuyer = false, onsidebarToggle }: Props = $props();
	...
	<div class="flex items-center gap-3">
		...
		>
			<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
				{#if sidebarOpen}
					<path d="M21 3a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h18Zm-1 2H9v14h11V5ZM7 5H4v14h3V5Zm6 4v6l-3-3 3-3Z" />
				{:else}
					<path d="M21 3a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h18Zm-1 2H9v14h11V5ZM7 5H4v14h3V5Zm4 4 3 3-3 3v-6Z" />
				{/if}
			</svg>
		</button>
		<div class="flex items-center gap-2">
			...
			<div class="hidden flex-col sm:flex">
				<span class="text-sm font-semibold leading-tight">{orgDisplayName}</span>
				<span class="text-[10px] font-mono text-muted-foreground dark:text-muted-foreground text-zinc-500">Powered by Threadline</span>
			</div>
		</div>
	</div>
	...
	<div class="flex items-center gap-2">
		...
		{#if isBuyer}
			...
			>
				...
				{#if cartCount > 0}
					<span class="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-bold text-primary-foreground">{cartCount > 99 ? '99+' : cartCount}</span>
				{/if}
			</a>
		{/if}
		...
		<div class="relative">
			...
			>
				<Avatar class="h-8 w-8">
					<AvatarFallback class="text-[11px] font-semibold bg-primary text-primary-foreground">
				...
				{#if user}
					<div class="hidden sm:flex flex-col items-start leading-tight">
						...
						{#if role}
							<span class="text-[11px] font-mono text-muted-foreground">{roleLabel(role)}</span>
						{/if}
					</div>
					<svg xmlns="http://www.w3.org/2000/svg" class="hidden h-4 w-4 text-muted-foreground sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			{#if showUserMenu}
				...
				<div class="absolute right-0 z-50 mt-1 w-64 animate-in rounded-none border bg-popover p-1.5 shadow-lg">
					<a href="/settings" class="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-accent" onclick={() => (showUserMenu = false)}>
					...
					{#if showTeam}
						<a href="/organization/members" class="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-accent" onclick={() => (showUserMenu = false)}>
							<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
					...
					<button class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-accent" onclick={signOut}>
