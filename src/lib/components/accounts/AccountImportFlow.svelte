<script lang="ts">
	...
	import { parseCSV as parseAccountCSV } from '$lib/utils/csv-parse.js';
	import {
		SCANNING_MIN_MS,
		PREPARING_MIN_MS,
		...
		ensureMinElapsed
	} from './account-import-helpers.js';
	import type { AccountImportResult } from '$lib/schemas/account-import.js';

	// Account import flow: tabs (Upload | Paste CSV) → loading stages →
	// preview list → Save Items / Continue. CSV-only — no PDF/AI path
	// (linesheets don't apply to accounts) and no season/year batch
	// fields. Mirrors <ProductImportFlow> visually so /accounts and
	// /products feel like the same product.
	type Props = {
		commitLabel?: string;
		commitPendingLabel?: string;
		// Fires after the import POST succeeds. Receives the server's
		// AccountImportResult so the parent can toast the count.
		onComplete: (result: AccountImportResult) => void;
	};
	...
	async function handleFile(file: File) {
		...
		try {
			...
			const { headers, rows } = parseAccountCSV(text);
			if (rows.length === 0) {
			...
			const built = buildAccountPreviewFromCsv(headers, rows);
	...
	async function handlePaste() {
		...
		const scanStart = Date.now();
		try {
			const { headers, rows } = parseAccountCSV(pasteValue);
			if (rows.length === 0) {
				errorMessage = 'No rows found. Make sure your CSV has a header row.';
				return;
			}
	...
	async function commitImport() {
		...
		try {
			const res = await fetch('/api/accounts/import', {
				...
				body: JSON.stringify({ accounts: previewRows })
			});
			const body = (await res.json()) as AccountImportResult & { error?: string };
			if (!res.ok) {
	...
	<div class="space-y-3">
		...
		{#if activeTab === 'upload'}
			...
			{:else}
				...
				<label
					for="account-import-file-input"
					aria-label="Upload accounts file"
		...
		<div class="flex items-center justify-between text-sm text-muted-foreground">
			<button
				type="button"
				onclick={() => downloadAccountCsvTemplate()}
...
{:else}
	...
	<div class="space-y-3">
		<div class="flex items-center justify-between">
			...
			<p class="text-sm text-muted-foreground">
				Showing {Math.min(previewRows.length, 10)} of {previewRows.length} accounts
			</p>
		</div>
			...
			{#each previewRows.slice(0, 10) as row, i (i)}
				<li class="flex items-center gap-4 rounded-md border p-3">
					<div class="min-w-0 flex-1 truncate text-sm">
						<span class="font-medium">{(row.business_name as string) || 'Untitled'}</span>
						{#if row.contact_email}
							<span class="text-muted-foreground"> · {row.contact_email as string}</span>
						{/if}
					</div>
