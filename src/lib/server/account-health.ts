...
export async function computeAccountHealth(
	...
	const accountMetrics = new Map<string, {
		orders: Array<{ total_amount: number; created_at: string; order_year: number | null; status: string }>;
		createdAt: string;
		isActive: boolean;
	...
	for (const [accountId, metrics] of accountMetrics) {
		...
		const ytdRevenue = acctOrders.filter(o => o.order_year === currentYear).reduce((sum, o) => sum + o.total_amount, 0);
		const priorYearRevenue = acctOrders.filter(o => o.order_year === priorYear).reduce((sum, o) => sum + o.total_amount, 0);
		...
		const daysSinceLastOrder = lastOrderTime ? Math.floor((now.getTime() - lastOrderTime) / (1000 * 60 * 60 * 24)) : null;

		// YoY growth
		const yoyGrowth = priorYearRevenue > 0
			? ((ytdRevenue - priorYearRevenue) / priorYearRevenue) * 100
			: null;

		// Account age in days
		const accountAgeDays = Math.floor((now.getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60 * 24));
		...
		if (accountAgeDays <= 90 && totalOrders === 0) {
			healthMap.set(accountId, {
				accountId, score: 0, label: 'new',
				signals: ['New account — no orders yet'],
				lastOrderDate, daysSinceLastOrder, totalOrders, ytdOrders,
				lifetimeRevenue, ytdRevenue, avgOrderValue, yoyGrowth
			});
			continue;
		}
		...
		if (totalOrders === 0) {
			healthMap.set(accountId, {
				accountId, score: 10, label: 'inactive',
		...
		if (daysSinceLastOrder !== null) {
			...
			else if (daysSinceLastOrder <= 60) { score += 25; }
			else if (daysSinceLastOrder <= 90) { score += 20; }
			else if (daysSinceLastOrder <= 180) { score += 10; signals.push(`Last order ${daysSinceLastOrder} days ago`); }
			else { score += 5; signals.push(`No orders in ${daysSinceLastOrder} days`); }
		}

		// Frequency score (0-25 points)
		if (ytdOrders >= 5) { score += 25; }
		else if (ytdOrders >= 3) { score += 20; }
		else if (ytdOrders >= 1) { score += 15; }
		else if (totalOrders >= 3) { score += 5; signals.push('No orders this year'); }
		else { signals.push('No orders this year'); }

		// Monetary score (0-25 points)
		if (ytdRevenue >= 50000) { score += 25; }
		else if (ytdRevenue >= 20000) { score += 20; }
		else if (ytdRevenue >= 5000) { score += 15; }
		else if (ytdRevenue >= 1000) { score += 10; }
		else if (ytdRevenue > 0) { score += 5; }

		// Growth score (0-20 points)
		if (yoyGrowth !== null) {
			if (yoyGrowth >= 20) { score += 20; signals.push(`Growing ${Math.round(yoyGrowth)}% YoY`); }
			else if (yoyGrowth >= 0) { score += 15; }
			else if (yoyGrowth >= -20) { score += 10; signals.push(`Down ${Math.abs(Math.round(yoyGrowth))}% YoY`); }
			else { score += 5; signals.push(`Declining ${Math.abs(Math.round(yoyGrowth))}% YoY`); }
		} else if (ytdOrders > 0) {
